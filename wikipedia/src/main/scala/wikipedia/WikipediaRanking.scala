...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    def getLang(article: WikipediaArticle): String = langs.find(lang => article.mentionsLanguage(lang)).getOrElse("None")

    rdd.map(article => (getLang(article), article)).groupByKey()
  }
